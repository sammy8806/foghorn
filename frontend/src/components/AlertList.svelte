...
<div class="alert-list-container">
  ...
  <div class="filter-bar" bind:this={filterBarEl}>
    ...
    >
      ...
      />
      {#if hasSearchText}
        <button class="search-clear" title="Clear search" aria-label="Clear search" on:click|stopPropagation={clearSearch}>×</button>
      {/if}
    </div>
    ...
    <button
      ...
      aria-label="Toggle verbose display"
  ...
  <div class="status-bar">
    ...
    {:else}
      ...
      {#if newVisibleCount > 0}
        <button class="status-chip status-chip-new" title="New alerts stay highlighted until you hover or focus them briefly. Click to mark all as seen." aria-label="Mark {newVisibleCount} new alerts as seen" on:click={acknowledgeAllAlerts}>
          <span class="status-chip-x" aria-hidden="true">×</span>
          {newVisibleCount} New
        </button>
      {/if}
      {#if resolvedVisibleCount > 0}
        <button class="status-chip status-chip-resolved" title="Resolved alerts stay visible for 30 seconds, or until you mark them seen. Click to clear them now." aria-label="Clear {resolvedVisibleCount} resolved alerts" on:click={acknowledgeAllResolvedAlerts}>
          <span class="status-chip-x" aria-hidden="true">×</span>
